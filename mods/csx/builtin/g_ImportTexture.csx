		...
		foreach (FileInfo fi in files)
		{
			...
			else if (ext == ".png")
			{
				...
				if (stripMatch is not null && stripMatch.Success)
				{
					...
					{
						Log.Error(fullName + " has a width not divisible by the number of frames.");
						throw new Exception();
					}
	...
	foreach (string file in dirFiles)
	{
		...
		if (spriteType == SpriteType.Sprite)
		{
			...
			try
			{
				...
				var regexForSpriteName = new Regex(@"^(.*?)(?=_f[0-9])");
				...
				{
					// If no match, fallback to original stripped value
					spriteName = stripped;
				}
				spriteList.Add(spriteName);
				//Log.Information($"stripped is {stripped} SpriteName is {spriteName}");
				...
				foreach (Match match in matches)
				{
					if (match.Groups[1].Success)
						...
						frameSpeed = int.Parse(match.Groups[5].Value);
					else if (match.Groups[6].Success)
				...
				//Log.Information($"{spriteName}: x={xCoordinate} y={yCoordinate}");
			}
			catch (Exception e)
			{
